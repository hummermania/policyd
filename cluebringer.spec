...
install -m 755 -o root -g root cbpolicyd cbpadmin database/convert-tsql $RPM_BUILD_ROOT%{_sbindir}
install -m 644 -o root -g root cluebringer.conf $RPM_BUILD_ROOT%{_sysconfdir}/cluebringer.conf
install -m 755 -o root -g root contrib/initscripts/Fedora/cbpolicyd $RPM_BUILD_ROOT%{_initrddir}

cp -R webui/* $RPM_BUILD_ROOT%{_datadir}/%{name}
install -m 644 -o root -g root contrib/httpd/cluebringer.conf $RPM_BUILD_ROOT%{apacheconfdir}/cluebringer.conf

install -m 644 -o root -g root AUTHORS INSTALL LICENSE TODO WISHLIST $RPM_BUILD_ROOT%{_docdir}/%{name}-%{version}
cp -R contrib $RPM_BUILD_ROOT%{_docdir}/%{name}-%{version}/contrib/amavisd-new
install -m 644 -o root -g root database/*.sql $RPM_BUILD_ROOT%{_docdir}/%{name}-%{version}/database

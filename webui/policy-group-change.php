...
if ($_POST['frmaction'] == "change") {
	...
	if (isset($_POST['policy_group_id'])) {
		# Prepare statement
		$stmt = $db->prepare('SELECT ID, Name, Disabled FROM policy_groups WHERE ID = ?');
?>
...
<?php
	...
	if (sizeof($updates) > 0) {
		...
		$res = $db->exec("UPDATE policy_groups SET $updateStr WHERE ID = ".$db->quote($_POST['policy_group_id']));
		if ($res) {
?>
